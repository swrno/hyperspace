...
| **hypr-sdk** | — | A TypeScript client (`packages/hypr-sdk`) that lets any external application query a hypr app's Knowledge Base and get memory-personalized answers for its own end-users. |
...
- **`/api/sdk/query`** — what a real end-user talks to, via `hypr-sdk`,
...
- Integrating a third-party app? Start with [hypr-sdk: Getting started](/sdk/getting-started).
