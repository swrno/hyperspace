...
export async function refresh(refreshToken) {
  ...
  if (!res.ok) {
    const err = new Error(`Jira refresh failed (${res.status})`);
...
export async function accessibleResources(token) {
  ...
  return res.json(); // [{ id, name, url, scopes }]
}
...
export async function me(token) {
  ...
  return res.json(); // { account_id, email, name }
}
...
async function paginate(cloudId, path, token, params = {}, { itemsKey, maxPages = 20 } = {}) {
  const out = [];
  let startAt = 0;
