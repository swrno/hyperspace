...
import {
  ...
  Bot, User, Check, Plus, Settings, Edit2, MessageSquare,
...
const modeIcon = (id, size = 14, color = '#C9A66B') => {
  const s = size;
  if (id === 'deep') {
    return (
...
const generateSarcasticReasoning = (prompt, responseText) => {
...
const ReasoningContent = ({ children }) => {
  const context = React.useContext(ReasoningContext);
  if (!context) return null;
...
function App() {
  ...
  const [activeScreen, setActiveScreen] = useState('dashboard'); // 'dashboard' | 'chat' | 'knowledge' | 'integrations' | 'admin'
  const [adminUsers, setAdminUsers] = useState([]);
  const [adminSearch, setAdminSearch] = useState('');
  const [adminLoading, setAdminLoading] = useState(false);
  const [adminUpdatingUid, setAdminUpdatingUid] = useState(null);
  const [activeDropdownUid, setActiveDropdownUid] = useState(null);
  ...
  const mockupMessagesEndRef = useRef(null);
...
Actually, wait - I should check if they already have any auth setup. Let me reconsider, they probably don't even know what Firebase is.`;
  ...
  const [currentModel, setCurrentModel] = useState('normal');
  ...
  const [settings, setSettings] = useState({ model: 'gemini-2.5-flash', temperature: 0.7 });
  const [openMenuId, setOpenMenuId] = useState(null);
  const [renameModal, setRenameModal] = useState(null);
  const [deleteModal, setDeleteModal] = useState(null);
  const [clearAllModal, setClearAllModal] = useState(false);
  // Connector / knowledge-graph ingestion state
  const [connectors, setConnectors] = useState({}); // { [platformId]: { connected, account, selectedItems, status, lastSync } }
  const [isConnectorsOpen, setIsConnectorsOpen] = useState(true);
  const [connectorModal, setConnectorModal] = useState(null); // open platform id
  const [connectorStage, setConnectorStage] = useState('auth'); // 'auth' | 'select' | 'ingest'
  const [connectorBusy, setConnectorBusy] = useState(false);
  const [availableItems, setAvailableItems] = useState([]);
  const [selectedItemIds, setSelectedItemIds] = useState([]);
  const [itemQuery, setItemQuery] = useState('');
  const [ingestProgress, setIngestProgress] = useState({}); // { [itemId]: 'queued'|'ingesting'|'synced' }
  ...
  const connectorsCacheKey = (token) => `hs_connectors_${uidFromToken(token)}`;
  const persistConnectorsLocal = (next, token) => {
    try { localStorage.setItem(connectorsCacheKey(token || idToken), JSON.stringify(next)); } catch { /* ignore */ }
  };

  const loadConnectors = async (token) => {
  ...
  const saveConnector = async (platformId, payload) => {
  ...
  const fetchItems = async (platformId, preselect = []) => {
    setConnectorBusy(true);
    try {
      let items = [];
      if (idToken) {
  ...
  const authorizePlatform = async (platformId) => {
    setConnectorBusy(true);
  ...
  const startIngestion = (platformId) => {
    ...
    chosen.forEach((item, idx) => {
      setTimeout(() => {
        setIngestProgress(prev => ({ ...prev, [item.id]: 'ingesting' }));
      }, 400 + idx * 650);
      setTimeout(() => {
        setIngestProgress(prev => ({ ...prev, [item.id]: 'synced' }));
      }, 400 + idx * 650 + 900);
    });
  ...
  const recentSends = useRef([]);

  // Cloud Database Sync Helper
  const syncChatToDb = async (chatToSync, tokenToUse) => {
  ...
  const updateAdminUser = async (targetUid, tier, customHourLimit) => {
    if (!idToken) return;
  ...
  const getChatDateGroupText = (isoString) => {
  ...
  const copyUserMessage = (text, id) => {
    navigator.clipboard.writeText(text);
    setCopiedId(id);
    setTimeout(() => setCopiedId(null), 2000);
  };

  const handleRetryUserMessage = (msg) => {
  ...
  const handleRetryAiMessage = (aiMsg) => {
    if (!currentChatId || isLoading) return;
    const chat = chats.find(c => c.id === currentChatId);
    if (!chat) return;
  ...
  const handleSaveEdit = (msg, newText) => {
  ...
  const handleSendDirectlyWithHistory = async (messageText, clearedHistory) => {
    if (isLoading) return;
    const now = Date.now();
      ...
      <div className={`flex flex-col gap-0.5 ${isSidebarCollapsed ? 'mt-4 items-center' : 'px-3 mt-1'}`}>
        {[
          ...
          return (
            <button
              key={id}
              onClick={() => { setActiveScreen(id); setIsSidebarOpen(false); }}
              title={label}
                      ...
                      <div key={msg.id} className="flex flex-col w-full animate-message-appear">
                        {msg.role === 'user' ? (
                          ...
                          <div className="flex flex-col">
                            ...
                            <div className="text-[#F4F0EB] font-basel text-[15px] leading-relaxed" style={{ letterSpacing: '0.2px' }}>
                              <ReactMarkdown
                                ...
                                components={{
                                  ...
                                  ul: ({ children }) => <ul className="list-disc pl-5 my-3 space-y-1.5 marker:text-[#6B6762]">{children}</ul>,
                                  ol: ({ children }) => <ol className="list-decimal pl-5 my-3 space-y-1.5 marker:text-[#6B6762]">{children}</ol>,
                                  li: ({ children }) => <li className="leading-relaxed pl-1">{children}</li>,
                                  h1: ({ children }) => <h1 className="text-[18px] font-geist font-semibold text-[#F4F0EB] tracking-tight mt-5 mb-2.5 first:mt-0">{children}</h1>,
                                  h2: ({ children }) => <h2 className="text-[16px] font-geist font-semibold text-[#F4F0EB] tracking-tight mt-5 mb-2 first:mt-0">{children}</h2>,
                                  h3: ({ children }) => <h3 className="text-[14px] font-geist font-semibold text-[#F4F0EB] tracking-tight mt-4 mb-1.5 first:mt-0">{children}</h3>,
                                  strong: ({ children }) => <strong className="font-semibold text-[#F4F0EB]">{children}</strong>,
                                  a: ({ children, href }) => <a href={href} target="_blank" rel="noreferrer" className="text-[#D8B48C] underline underline-offset-2 decoration-[#5A5048] hover:decoration-[#D8B48C] transition-colors">{children}</a>,
                                  blockquote: ({ children }) => <blockquote className="border-l-2 border-[#4A4744] pl-3.5 my-3 text-[#C7C2BC] italic">{children}</blockquote>,
                                  table: ({ children }) => <div className="my-4 overflow-x-auto rounded-xl border border-[#33302E]"><table className="w-full text-[13px] border-collapse">{children}</table></div>,
                                  th: ({ children }) => <th className="bg-[#211F1D] text-left font-geist font-semibold text-[#C7C2BC] px-3.5 py-2 border-b border-[#33302E]">{children}</th>,
                                  td: ({ children }) => <td className="px-3.5 py-2 border-b border-[#2A2826] text-[#C7C2BC]">{children}</td>,
                                }}
                              >
                                {msg.content}
                              </ReactMarkdown>
