...
import {
  ...
  Network, FileText, Loader2, Activity,
} from 'lucide-react';
...
export default function Dashboard({ user, idToken, connectors = {}, onNavigate, onAsk, platformIcon }) {
  ...
  const icon = (id, size) => (platformIcon ? platformIcon({ id }, size) : null);

  const liveByProvider = useMemo(
    () => Object.fromEntries((stats?.connections || []).map((c) => [c.provider, c])),
    [stats]
  );
  const realBySource = useMemo(
  ...
  const submitAsk = (q) => { const t = (q ?? query).trim(); if (t) onAsk?.(t); };
