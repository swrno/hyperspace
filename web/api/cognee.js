...
export async function addText(text, { userId, nodeSet } = {}) {
...
export async function cognify(userId, { force = false } = {}) {
...
export async function ingest(text, { userId, nodeSet } = {}) {
 ...
 */
export async function graphSearch(query, { userId, searchType = 'GRAPH_COMPLETION', topK = 10 } = {}) {
...
export async function getDatasetGraph(userId) {
  ...
  try {
    ...
    if (!res.ok) return null;
    return res.json(); // { nodes:[{id,label,type,properties}], edges:[...] }
...
export async function rememberMemory(text, { userId } = {}) {
  if (!configured() || !text?.trim()) return null;
  try {
...
export async function recallMemory(query, { userId } = {}) {
  if (!configured() || !query?.trim()) return null;
  try {
...
export async function rememberText(text, opts = {}) {
