 ...
 * Sourced live from Neo4j (getUserGraphStats, Cypher) when the graph has data
...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    const kbDocuments = kbs.reduce((s, k) => s + (k.documents?.length || 0), 0);

    let payload: Omit<NonNullable<typeof graphStats>, 'documents'> & { documents: number };
    if (graphStats && graphStats.total > 0) {
      payload = { ...graphStats, documents: graphStats.documents || kbDocuments };
    } else {
      // Neo4j graph is empty for this user (e.g. pre-migration account) — fall
      // back to the legacy Mongo kb_entities structural index.
      const kb = db.collection('kb_entities');
      const since = new Date(Date.now() - 13 * 24 * 60 * 60 * 1000).toISOString();
      const [byType, bySource, byStatus, timelineRaw, recent, allEnts] = await Promise.all([
      ...
      payload = {
        total: byType.reduce((s, r) => s + r.n, 0),
        graph: buildStructuralGraph(allEnts).stats,
