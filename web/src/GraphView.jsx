...
const communityColor = (i) => COMMUNITY_COLORS[((i % COMMUNITY_COLORS.length) + COMMUNITY_COLORS.length) % COMMUNITY_COLORS.length];
...
const colorOf = (t) => TYPE_COLOR[t] || '#8C8880';
const sizeOf = (n) => (n.type === 'Source' ? 13 : n.type === 'Person' ? 7 : 5 + Math.min(n.degree || 0, 9));
...
function SigmaGraph({ data, hideTypes, onPick, colorFor }) {
  ...
  useEffect(() => {
    registerEvents({
      ...
      clickNode: (e) => { if (!movedRef.current) onPick(sigma.getGraph().getNodeAttribute(e.node, 'raw')); },
      enterNode: (e) => { hoveredRef.current = e.node; document.body.style.cursor = 'grab'; sigma.refresh(); },
...
function Graph3D({ data, hideTypes, onPick, colorFor }) {
...
export default function GraphView({ idToken, onAsk }) {
  const [view, setView] = useState('2d'); // '2d' | '3d'
  const [mode, setMode] = useState('structural'); // 'structural' | 'cognee'
  const [colorBy, setColorBy] = useState('type'); // 'type' | 'community'
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [syncing, setSyncing] = useState(false);
  const [hideTypes, setHideTypes] = useState(() => new Set());
  ...
  const load = async (m = mode) => {
    ...
    } catch (e) { console.warn('graph load failed', e.message); }
    ...
    <div className="flex-1 flex flex-col h-full bg-[#252523] font-geist overflow-hidden animate-fade-in">
      ...
      <div className="flex-1 relative overflow-hidden">
        {loading ? (
          ...
          <Graph3D data={data} hideTypes={hideTypes} onPick={setSelected} colorFor={colorFor} />
        ) : (
          <SigmaContainer
          ...
          >
            <SigmaGraph data={data} hideTypes={hideTypes} onPick={setSelected} colorFor={colorFor} />
          </SigmaContainer>
        )}
